...
| homeassistant | 8123/6053 | Central automation hub + voice intents. **Version: 2026.4.4** (updated 2026-05-02). **Known bug:** Spotify coordinator throws `MissingField: Field "items" of type PlaylistTracks is missing` on every poll cycle (~every 10–20s) — caused by Spotify's Feb 2026 API change removing `/playlists/{id}/tracks`. Causes playback lag (error loop hammers API). No fix as of 2026.4.4. See TROUBLESHOOTING.md. Workaround: avoid Spotify algorithmic playlists (Daily Mix, Radio, Discover Weekly). |
