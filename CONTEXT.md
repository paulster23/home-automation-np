...
- **One camera:** `front_window` @ 192.168.1.33 (Reolink E1 Pro, 2.4 GHz WiFi, no ethernet port; 4 zones — entrance, sidewalk, active_street, northside; tracks person + car). The `backyard` RLC-510WA (`.248`) was removed from this config 2026-09-08 — it is now LIVE at New Paltz as `porch` (in-camera static 192.168.2.34, SSID `sinola 2.4`) in the NP Frigate's own `frigate/config.yml` — there is no `config.np.yml`. Brought up 2026-09-18: sub-stream only (640x480) detect+record; HA sees it via native MQTT entities (device "Porch Camera (Frigate)"); RTSP/ONVIF had to be enabled in the Reolink app (off by default). It had been `enabled: false` since 2026-08-05, so Brooklyn has been a one-camera site in practice since then.
