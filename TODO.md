...
### ~~Remove spotcast integration~~ — ✅ Done 2026-04-22 / 2026-05-06
Removed via HACS 2026-04-22. `configuration.yaml` spotcast block was already commented out. `secrets.yaml` spotcast keys removed. **Note:** HACS removal does not delete component files from disk — `custom_components/spotcast/` remained, causing a loader warning on every HA startup. Files manually deleted 2026-05-06 (`rm -rf ~/containers/home-automation/homeassistant/custom_components/spotcast/`). HA restarted clean — spotcast no longer appears in loader warnings.
...
- [x] **Fix "Stop" misheard as "Pause"** — `pause` already present as a bare `StopMusic` alias in `custom_sentences/en/music.yaml` (confirmed 2026-05-06). No change needed.
