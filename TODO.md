...
- [ ] **Naboo silent after librespot heal — stream doesn't reconnect (2026-06-09)** — After `librespot_heal.sh` (manual or via `librespot_monitor.py` auto-heal), pressing spotify_resume starts Spotify-side playback and the `librespot_playing` webhook + play automation fire, but the Voice PE's pull of `http://192.168.1.70:8765` dies (`BrokenPipeError` in HA log) — symptom: phone shows "playing on Naboo", no audio. Manual play in HA fixes it. Likely race: Voice PE connects to `serve_http.py` before audio flows post-restart. Candidate fixes: heal script re-sends `librespot_playing` webhook ~10s after new PID when Spotify is playing; or extend `librespot_stream_resilience` automation (check why it didn't recover — `mode: single` + satellite condition?). Diagnostic: `streamdeck/spotify_resume.last.log` now captures every button press. Related: go-librespot investigation (Future) would eliminate this class of bug.
